// Adapted from the MIT VGPU Atmosphere example. Use storage dimensions so
// a website background can bake the same noise field at a smaller resolution.
import { perlinFbm, remap, worley3 } from "../../atmosphere/noise-common.wgsl";
...
fn main(@builtin(global_invocation_id) id: vec3u) {
  let p = (vec3f(id) + 0.5) / f32(textureDimensions(shapeNoise).x);
