    ...
    try {
      ...
      const rain = vgpu.draw(gpu, {
        ...
        set: {
          ...
          params: { size: [...surface.size], time: simulationTime, shutter: 1 / 45 },
        },
      });
      const lens = vgpu.effect(gpu, shaders.lens, {
        ...
        set: {
          ...
          params: { size: [...surface.size], time: simulationTime, wetness: 0.14 },
        },
      });
      ...
      function render(frame) {
        ...
        try {
          ...
          if (frameCount === 1) onReady({ backend: 'vgpu-webgpu', camera: 'fixed-upward' });
