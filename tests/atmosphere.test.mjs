...
test('camera is identical at every hour', () => {
  for (let hour = 0; hour < 24; hour++) {
    ...
    assert.equal(state.yaw, 0); assert.equal(state.pitch, 78); assert.equal(state.altitudeKm, .08);
  }
});
...
test('uses the official graph followed by the GPU rain passes', async () => {
  ...
  assert.equal(h.states[0].pitch, 78); assert.equal(h.effects.length, 3); stop();
});
